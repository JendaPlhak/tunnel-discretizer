...
def proj_dist(d1, d2):
    if (d1.normal == d2.normal).all():
        return numpy.linalg.norm(d1.center - d2.center)
    else:
...
#   sVis = sphere(pos = (s[0], s[1], s[2]), radius = s[3], opacity=0.3)
#   # central line
#   if (i < len(tunnel)-1):
#     s2 = tunnel[i+1]
#     vVis = arrow(pos=(s[0], s[1], s[2]), axis=(s2[0]-s[0], s2[1]-s[1], s2[2]-s[2]), color=(1,0,0), shaftwidth=1)
# infile.close()

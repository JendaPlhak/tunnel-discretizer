    ...
    \For{$ S_i \in \Tau $}
        ...
        \While {True}
            ...
            \If{$\operatorname{makesSharpTurn}(prev\_disk, curve)$}
                \State $ disk\_center
                    \gets prev\_disk^{center} + prev\_disk^{normal} * \delta $ \label{alg:shift_by_delta}
            ...
            \Else
                \State $ disk\_center \gets prev\_disk^{center} + prev\_disk^{normal} * \epsilon $
...
Nakonec na řádcích \ref{digTunnel:last_if} až \ref{digTunnel:last_if_end}n
...
podobě pro prakticky libovolné dimenze. Binding je veřejně dostupný z reference
uvedené v literatuře (\cite{python_balls}).
...
\Function{shiftDisk}{$ V, W$}
    ...
    \If {$ \dst(V, new\_disk) > \delta \: || \: \neg \operatorname{isFollower}(prev\_disk, new\_disk)$ }
...
\subsection{Shift sharp turn} \label{subsec:shift_sharp_turn}
...
v projekci do nějaké roviny. V tomto případě však budou mít oba řezy stejný
...
středy úseček vzniklých projekcí řezů do této roviny. Také díky tomu, že
...
Nyní když máme k dispozici projekční rovinu můžeme podobně jako v případě
předchozí procedury $ \operatorname{shiftDisk} $ najít korespondující
vrcholy $ P_1, P_2 $ resp. $ Q_1, Q_2 $. Pomocí nich následně zjistíme, která
...
$ P_i $. Nakonec ještě na základě takto posunutých vrcholů vygenerujeme nový řez.

Uvedený postup je zachycen následujícím pseudokódem.
...
\Function{shiftSharpTurn}{$ V, W$}
    ...
    \State $ \vec{w_1} \gets Q_1 - P_1 $
    \If {$ \norm{w_1} > \delta $}
        $ Q_1 \gets P_1 + \tilde{\epsilon} \frac{w_1}{\norm{w_1}} $ \label{shiftSharpTurn:correction1}
    \EndIf

    \State $ \vec{w_2} \gets Q_2 - P_2 $
    \If {$ \norm{w_2} > \delta $}
        $ Q_2 \gets P_2 + \tilde{\epsilon} \frac{w_2}{\norm{w_2}} $ \label{shiftSharpTurn:correction2}
    \EndIf
    \Statex

    \State $ Z \gets $ Construct disk from $ Q_1$, $Q_2$ perpendicular to $ \rho $
    \State $ new\_disk \gets \operatorname{fitDiskTunnel}(Z_{normal}, Z_{center}) $ \label{shiftSharpTurn:fit}
    \Statex

    \If {$ \dst(V, new\_disk) > \delta \: || \: \neg \operatorname{isFollower}(prev\_disk, new\_disk)$ }
        \State \Return $ \operatorname{shiftSharpTurn}(V, new\_disk) $ \label{shiftSharpTurn:recursion}
    \Else
        \State \Return $ new\_disk $
    \EndIf

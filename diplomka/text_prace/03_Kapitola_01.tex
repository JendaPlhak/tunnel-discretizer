...
případech případech. Podrobně se jí budeme věnovat v podkapitole \ref{subsec:shift_bend}.
    ...
    \For{$ S_i \in \Tau $}
        ...
        \While {True}
            ...
            \If{$\operatorname{makesSharpTurn}(prev\_disk, curve)$}
                \State $ disk\_center \gets prev\_disk^{center} + prev\_disk^{normal} * \delta $
                \State $ disk\_normal \gets prev\_disk^{normal} $
...
průnikem roviny dané předchozím řezem a přímkou $ line $. Pokud je průnikem
jednoprvková množina, pak nám podmínky \ref{cond:not_intersecting} a
\ref{cond:going_forward} garantují, že se $ d $ zvětší nejméně o $ \epsilon $,
čímž máme garantováno, že algoritmus bude konečný. V opačném případě je normála
...
Algoritmus tak bude jistě korektní. Konečnost plyne z toho, že

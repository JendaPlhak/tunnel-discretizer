...
$ \theta_i $. To zajistíme podmínkou \ref{cond:good_going}.
...
Kostru naznačeného algoritmu zachycuje následující pseudokód. Poznamenejme, že
budeme značit $ \operatorname{norm}(v) = \frac{v}{\norm{v}}$.
...
\Function{digTunnel}{$\Tau, \delta$}
    ...
    \For{$ S_i \in \Tau $}
        ...
        \While {$ size < \norm{dir} $}
            \If{$ size \neq 0 $}
                \State $ plane \gets \operatorname{getPlane}(disks[|disks|-1]) $
                \State $ size \gets \dis(plane \cap line, S_{i}^{center}) + \epsilon $
            \EndIf
            \Statex

            \State $ disk\_center \gets S_{i}^{center} + normal * size $
            ...
            \If{$ |disks| \geq 2  \And \dst(disk, disks[|disks|-2]) < \delta $}
                \State $ disks[|disks|-1] \gets disk $
            \Else

...
\KlicovaSlova{
    Diskretizace tunelu v proteinu; Molekulární doking; Doking ligandu; Trajektorie ligandu;
    Algoritmus; Enzymatická reakce; Výpočetní chemie; Heuristika
}{
...
\Abstrakty%
{
    V této diplomové práci se věnujeme popisu vybraných částí nového algoritmu pro analýzu
    a výpočet spojité trajektorie ligandu skrze tunel v biomolekule, který vede
    z jejího povrchu k aktivnímu místu. Konkrétně rozebereme algoritmus pro diskretizaci
    tunelu a uvedeme výsledky vyhodnocení nad širokou škálou tunelů. Druhou část práce
    tvoří popis techniky zlepšující energetický profil trajektorie ligandu
    pro tunely s více úzkými hrdly.
}%
{
    In this thesis we introduce selected parts of novel algorithm for analysis and
    computation of continuous ligand trajectory through tunnel in biomolecule via
    which is ligand transported from molecule surface to its binding site. Specifically
    we discuss algorithm for tunnel discretization and evaluate its performance on
    wide variety of tunnels. Second part of this thesis introduces technique improving
    energetic profile of ligand trajectory which can be efficiently used for
    tunnels with multiple bottlenecks.
}

\TextPodekovani%
{
    Na tomto místě bych chtěl poděkovat prof. RNDr. Luďku Matyskovi, CSc. za odborné
    vedení diplomové práce a RNDr. Jiřímu Filipoviči, Ph.D. za velkou ochotu, vstřícnost,
    nespočet rad při vývoji algoritmů a možnost podílet se na takto zajímavém projektu.
    Také bych chtěl poděkovat doc. Mgr. Josefu Šilhanovi, Ph.D. za podnětné rozhovory
    na téma diferenciální geometrie a doc. RNDr. Martinu Čadkovi, CSc. za cenné rady.
}
...
\VlozZadani{text_prace/Zadani.pdf}
...
\HlavickaKapitoly
\vloz{text_prace/03_Kapitola_00}
\cleardoublepage
